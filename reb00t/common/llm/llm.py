...
def product_version_long():
    return "1.0.0"

from reb00t.common.utils.keychain import get_env
...
def create_client(model_id, model):
    ...
    if model_id == "4o":
        ...
        azure_api_key = get_env("AZURE_OPENAI_API_KEY_DE", 'azure-open-ai-api-key-4o', 'reb00t')
        #
        # azure_endpoint = "https://remedee-swe.openai.azure.com/" #os.environ["AZURE_OPENAI_ENDPOINT"]
        # azure_api_key = get_env("AZURE_OPENAI_API_KEY", 'azure-open-ai-api-swe', 'remedee')
    elif model_id == "4o-mini" and model["variant"] and "sense" in model["variant"]:
        azure_endpoint = "https://remedee-swe.openai.azure.com/" #os.environ["AZURE_OPENAI_ENDPOINT"]
        azure_api_key = get_env("AZURE_OPENAI_API_KEY_SWE", 'azure-open-ai-api-swe', 'remedee')
...
class LLM:
    ...
    def get_default_instance():
        if not hasattr(LLM, "_default_instance") or LLM._default_instance is None:
            LLM._default_instance = LLM(cache=True)
        return LLM._default_instance

    def __init__(self, model=None, cache_prefix=".cache/request_cache", cache=False, max_output_tokens = None, instance="default", variant=None):
        if model is None:
            model = "4o"
        ...
        if variant:
            self.model["name"] = f"{self.model['name']}-{variant}"
    ...
    async def _start_completion(self, model, messages, temperature=None, response_format=None):
    ...
    async def _stream_response(self, response, background, full_log_path, stream_callback=None, log_tokens=True):
        ...
        if event is not None and event.choices[0].finish_reason is not None:
            ...
            if finish_reason != "stop":
                extra = event.choices[0].model_extra if hasattr(event.choices[0], "model_extra") else "NA"
    ...
    async def _execute_query_retry(self, messages, input_tokens, background, full_log_path, stream_callback=None, temperature=None, log_response=True,
        ...
        try:
            response = await self._start_completion(model, messages, temperature, response_format=response_format)
    ...
    async def query(self, messages, cache=None, stream_callback=None, temperature=None, background=False, prompt_name=None, response_format=None):
        ...
        if response_text is None:
            ...
            response_text = await self._execute_query_retry(messages,
                                                            ...
                                                            stream_callback,
                                                            temperature,
                                                            log_response=log_stream,
                                                            response_format=response_format)   # _execute_query logs the response token by token
            if not log_stream:
    ...
    async def query_simple(self, query, response_format=None):
        messages = self.get_messages(query)
        return await self.query(messages, response_format=response_format)
